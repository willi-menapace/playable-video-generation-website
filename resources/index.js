...
async function reset() {
   const preprocessedData = preprocess(images_array[initial_frame_index], W, H);	
   frame = new onnx.Tensor(preprocessedData, 'float32', [3, H, W]);
...
function preprocess(data, width, height) {
  ...
  const dataProcessed = ndarray(new Float32Array(width * height * 3), [3, width, height]);
...
function display(ctx) {
   ...
   const frame_array = ndarray(frame.data, [3, W, H]);
